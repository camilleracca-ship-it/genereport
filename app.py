...
st.title("Générateur de rapports à partir d'un CSV")

uploaded_file = st.file_uploader("Veuillez déposer votre fichier en format CSV.", type=["csv"])
...
"""
    ...
    for i, (_, row) in enumerate(df_patient.iterrows(), start=1):
        ...
        class_acmg = row["CLASS_ACMG"]
        class_acmg = row["CLASS_ACMG"]
        if pd.isna(class_acmg):
            class_acmg = ""
        class_acmg = str(class_acmg).strip()
        if type_de_variant == "intronique" or type_de_variant == "splice-site":
            variant_sentence = f"Il est localisé en position {position} du site d'épissage."
        else :
            variant_sentence = f"Il est localisé + {localisation_du_domaine} de la protéine (position {position})."
        ...
        if population_data == "présent":
            population_sentence = f"Il est présent dans les bases de données de populations contrôles (gnomAD v4.1.0 : {allele_frequency})."
        else:
            population_sentence = "Il n'est pas présent dans les bases de données de populations contrôles (gnomAD v4.1.0)."
...
Ce variant concerne un acide aminé {conservation} conservé dans les comparaisons inter-espèces. {variant_sentence}
